export const HOURS_KEY = 'hongik-health-hours-v1'
...
export function readHours(storage = localStorage) {
  const raw = storage.getItem(HOURS_KEY)
...
export function writeHours(value, storage = localStorage) {
  if (!validHours(value))
    throw new Error('운영 시간 설정이 올바르지 않습니다. 시작·종료 시각을 확인해 주세요.')
  storage.setItem(HOURS_KEY, JSON.stringify(normalizeHours(value)))
}
